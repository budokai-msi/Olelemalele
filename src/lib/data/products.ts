// Static product data only - safe for client and server
// Database functions moved to lib/db/products.ts
...
export function getProductsByCollection(collection: string): Product[] {
  return products.filter(p => p.productCollection === collection)
}

// Get all unique collections
export function getCollections(): string[] {
  return Array.from(new Set(products.map(p => p.productCollection).filter(Boolean) as string[]))
}
